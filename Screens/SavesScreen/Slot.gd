...
func init(name:String, page_index:Vector2, hide_delete:bool = false, empty:bool = false):
	...
	if empty:
		set_data_time(0)
		$Button/VBoxContainer/Panel/DummySlot.visible = true
		$Button/VBoxContainer/DateLabel.visible = false
	else:
		$Button/VBoxContainer/Panel/DummySlot.visible = false
		$Button/VBoxContainer/DateLabel.visible = true
...
func set_save_name(savename):
	$Button/VBoxContainer/SaveNameLabel.text = savename
